...
run_instance() {
	...
	instanceIDtmp="$(aws ec2 run-instances --image-id $defaultAMIID --count $defaultCount --instance-type $defaultInstanceType --placement $defaultPlacement --key-name $defaultKey --user-data "$(cat ./lumiBash.sh  | base64 -w 0)" --security-groups $defaultSecurityGroup --query 'Instances[0].InstanceId')"
